...
def getNetwork(input_img, mask, edges, reuse=None):
    ...
    # # 统一维度为 [1, H, W, C]
    # if input_img.shape.ndims == 3:
    #     input_img = tf.expand_dims(input_img, 0)  # [H,W,C] → [1,H,W,C]
    # if mask.shape.ndims == 2:
    #     mask = tf.expand_dims(mask, -1)           # [H,W] → [H,W,1]
    # if mask.shape.ndims == 3:
    #     mask = tf.expand_dims(mask, 0)            # [H,W,1] → [1,H,W,1]
    # if edges.shape.ndims == 2:
    #     edges = tf.expand_dims(edges, -1)
    # if edges.shape.ndims == 3:
